#include"./Myspider.h"
void AddDNS(DNSNode** temp,char*ip,char*s){
	DNSNode * t2 = *temp;
...
char* GetFromDNS(DNSNode* head,char*domain){
	while(head!=NULL){
...
int GetIp(char*ip,const char*domain){
	//DNS 部分可以优化
	printf("----------getip-------------\n");
	char *temp = GetFromDNS(Dhead,domain);
	if(temp==NULL){
	struct hostent *host = gethostbyname(domain);
	if(host==NULL) return 0;
	inet_ntop(AF_INET,host->h_addr_list[0],ip,16);
	AddDNS(&Dhead,ip,domain);
	}else{
